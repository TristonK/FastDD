...
public class BrutalEvidenceSetBuilder implements EvidenceSetBuild {
    private final EvidenceSet evidenceSet;
    public BrutalEvidenceSetBuilder(PredicateBuilder predicateBuilder){
...
//            HashMap<LongBitSet, Long> clueSet = linearBuildClueSet(pliShards);
            ...
            HashMap<LongBitSet, Long> clueSet = brutalBuildClueSet(pliShards);
    ...
    private HashMap<LongBitSet, Long> brutalBuildClueSet(PliShard[] pliShards){
        ...
        for (int i = 0; i < pliShards.length; i++) {
            for (int j = i; j < pliShards.length; j++) {
                ...
                HashMap<LongBitSet, Long> partialClueSet = builder.buildClueSet();//在这里选择buildClueSet的方式
